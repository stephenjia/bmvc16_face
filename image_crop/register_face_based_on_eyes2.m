...
function Y = register_face_based_on_eyes2(X,eye,chin,size1)
...
x_leva = double(eye.x(1));
y_leva = double(eye.y(1));
% x_desna = double(eye.x(2));
% y_desna = double(eye.y(2));
x_chin = double(chin.x(1));
y_chin = double(chin.y(1));

kk = abs(y_leva-y_chin);
...
y_u = max(T_y-round(kk*0.5),1);
y_l = min(T_y+round(kk*1.1), size(Y,1));
x_f = max(T_x-round(kk*0.6),1);
x_s = min(T_x+round(kk*0.6),size(Y,2));

% y_u = max(T_y-round(kk*0.3),1);
% y_l = min(T_y+round(kk*1.3), size(Y,1));
% x_f = max(T_x-round(kk*0.8),1);
% x_s = min(T_x+round(kk*0.8),size(Y,2));
